...
An identifier is a string of characters corresponding to the Unicode General Category[fn:ucgc] *Lu* (an uppercase letter) or *Ll* (a
...
The event ~source~ reference is the entity which generates this event. In this case each event structure has the
identifiers of the source entity included.
...
The enumeration ~DistanceUnit~ in [[lst:sdml-ex-empty-enum]] is valid but incomplete.
...
The union ~VehicleClass~ in [[lst:sdml-ex-empty-union]] is valid but incomplete.

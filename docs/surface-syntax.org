...
A cardinality specification may include /cardinality constraints/, ...

- Ordering :: one of ~ordered~ or ~unordered~ (default).
- Uniqueness :: one of ~ordered~ or ~nonunique~ (default).

#+NAME: tbl:builtin-sequence-types
#+CAPTION: Built-in Sequence Types
| Type          | Ordered | Unique | Cardinality Expression |
|---------------+---------+--------+------------------------|
| ~Bag~ (default) | No      | No     | ~{0..}~                  |
| ~List~          | Yes     | No     | ~{ordered 0..}~          |
| ~Maybe~         | N/A     | N/A    | ~{0..1}~                 |
| ~OrderedSet~    | Yes     | Yes    | ~{ordered unique 0..}~   |
| ~Set~           | No      | Yes    | ~{unique 0..}~           |

See also section [[seq:library-sequence-types]] which describes the standard library operations on sequence types.

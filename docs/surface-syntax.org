...
2. If the reader has access to an SDML catalog file (For details of the catalog file format see appendix [[app:resolver-catalog]]), a matching catalog entry MUST be used as the module URI.
